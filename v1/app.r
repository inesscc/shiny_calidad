...
server <- function(input, output) {
  ...
  tabuladoOK =  eventReactive(input$actionTAB,{
    ...
    #v_cruce =  isolate({input$varCRUCE})
 ...
 #  }   
    ...
    #   parse_expr(enexpr(NULL))
    
    
    #"Media","Proporción","Suma variable Continua","Conteo casos"
    
    
    paste0("DENTRO ",v_interes,v_cruce)
 ...
 #    insumos = calidad::crear_insumos_prop(var = !!parse_expr(enexpr(v_interes)),dominios = !!parse_expr(enexpr(v_cruce)), disenio = dc) #
